...
function header(buf) {
  ...
  logger.info("unknownInfo: ", unknownInfo);
...
function doFile(name, cb, order) {
  ...
  logger.info("Target Dir:", dir);
  logger.info("Options:", order.toString());
  wu.get(
    name,
    (buf) => {
      ...
      order.includes("o")
        ? wu.addIO(logger.info.bind(logger), "Unpack done.")
