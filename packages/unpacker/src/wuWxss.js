...
function doWxss(dir, cb, mainDir, nowDir) {
    ...
    function makeup(data) {
      ...
      if (onlyTest) {
        ...
        if (!actualPure[data] && !blockCss.includes(wu.changeExt(wu.toDir(cssFile, frameName), ''))) {
          logger.debug('Regard ' + cssFile + ' as pure import file.')
  ...
  function runVM(name, code) {
    ...
    logger.debug('do css runVm: ' + name)
    vm.run(code)
  ...
  wu.scanDirByExt(dir, '.html', files => {
    let frameFile = ''
    if (fs.existsSync(path.resolve(dir, 'page-frame.html'))) frameFile = path.resolve(dir, 'page-frame.html')
    else if (fs.existsSync(path.resolve(dir, 'app-wxss.js'))) frameFile = path.resolve(dir, 'app-wxss.js')
    else if (fs.existsSync(path.resolve(dir, 'page-frame.js'))) frameFile = path.resolve(dir, 'page-frame.js')
    ...
    wu.get(frameFile, code => {
      ...
      if (code.indexOf('__COMMON_STYLESHEETS__') != -1) {
        ...
        commonStyles = ';var __COMMON_STYLESHEETS__ = __COMMON_STYLESHEETS__||{};' + commonStyles + ';__COMMON_STYLESHEETS__;'
      ...
      logger.debug('Guess wxss(first turn)...')
      preRun(dir, frameFile, mainCode, files, () => {
        frameName = frameFile
        onlyTest = true
        runOnce()
        onlyTest = false
        logger.debug('Import count info: %j', importCnt)
        for (let id in pureData)
          if (!actualPure[id]) {
            ...
            if (importCnt[id] <= 1) {
              ...
              logger.debug('Cannot find pure import for _C[' + id + '], force to save it in (' + newFile + ').')
              id = Number.parseInt(id)
              actualPure[id] = newFile
              cssRebuild.call({ cssFile: newFile }, id)()
            }
          }
        logger.debug('Guess wxss(first turn) done.\nGenerate wxss(second turn)...')
        runOnce()
        logger.debug('Generate wxss(second turn) done.\nSave wxss...')
...
module.exports = { doWxss, splitWxssCmd }
