...
function tryWxml(dir, name, code, z, xPool, rDs, ...args) {
  logger.debug('Decompile ' + name + '...')
  let state = [null]
  try {
    doWxml(state, dir, name, code, z, xPool, rDs, ...args)
    logger.debug('Decompile success!')
  } catch (e) {
    logger.debug('error on ' + name + '(' + (state[0] === null ? 'Main' : 'Template-' + state[0]) + ')\nerr: ', e)
...
module.exports = { doFrame, splitWxmlCmd }
